        ...
        Timer().schedule(object : TimerTask() {
            override fun run() {
                binaryRequestFacade.executeRequest(NotificationsBinaryRequest())?.notifications?.forEach { binaryNotification ->
                    if (PropertiesComponent.getInstance().getBoolean(storageKey(binaryNotification.id))) {
                        return;
                    }

                    val notification = Notification(BRAND_NAME, ICON, NotificationType.INFORMATION)
                    ...
                    binaryNotification.options?.stream()?.findFirst()?.ifPresent { o: NotificationOptions ->
                        notification.addAction(object : AnAction(o.key, null, AllIcons.Icons.Ide.NextStep) {
                            override fun actionPerformed(e: AnActionEvent) {
                                binaryRequestFacade.executeRequest(NotificationActionRequest(binaryNotification.id, o.key, binaryNotification.message, null))
                                notification.expire()
                            }
                        })
                    }
